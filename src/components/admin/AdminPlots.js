...
const AdminPlots = () => {
  ...
  useEffect(() => {
    fetch("https://server-side-05i1.onrender.com/api/plots")
      .then(res => res.json())
      .then(data => {
  ...
  const handleCreatePlot = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch("https://server-side-05i1.onrender.com/api/plots", {
        method: "POST",
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <form onSubmit={async (e) => {
                e.preventDefault();
                try {
                  const res = await fetch(`https://server-side-05i1.onrender.com/api/plots/${editPlot.id}`, {
                    method: 'PUT',
                    headers: { "Content-Type": "application/json" },
